...
function Interaction() {
    ...
    const getExpansions = (input) => {
        ...
        axios.post('/expand', {input:input, model_type:modelType})
        .then(response => {
            let curOutput = response.data['output'];
            console.log(curOutput);
            if (curOutput.length == 0) {
                curOutput.push({"text": "No expansions found", 
    ...
    const formattedOutput = output.map((item, idx) => {
        ...
        for (var i=0; i < input_token_idxs.length; i++) {
            ...
            displayText.push((<span>{text.slice(last_end_token_idx, 
                                                start_token_idx)}</span>));
            var end_token_idx = input_token_idxs[i][1];
            displayText.push((<span><b>
                              {text.slice(start_token_idx, end_token_idx)}
                              </b></span>));
    ...
    return (<div>
            ...
            <label class="box-label" style={{fontSize:"1rem"}}>Input</label>
            <textarea placeholder="Enter sentence here and hit enter to expand" onKeyDown={handleKeyDown} 
